// tb_top_asic.v — ASIC-variant regression for ecg_accelerator_top_asic.
// Identical to tb_top.v except the DUT uses the macro-friendly memories
// (input_sram_asic 4096x8, ping_pong_sram_asic 2x512x64). Proves the packed
// memory refactor is bit-exact (must reproduce 21/21 + same 5216 cycles).
// Differences vs tb_top.v: DUT module name + ping_pong internal-read helpers
// read 64-bit packed words instead of 16 per-channel arrays.
//
...
module tb_top_asic;
    ...
    ecg_accelerator_top_asic u_top (
        .clk          (clk),
        .rst          (rst),
        .rst_n        (rst_n),
    ...
    // ASIC variant: ping_pong_sram_asic packs 8 channels into one 64-bit word
    // per bank (mem_a/mem_b are [63:0] arrays). Channel ch occupies [ch*8+:8].
    function [7:0] read_mem_a;
        input integer ch;
        input integer pos;
        begin
            read_mem_a = u_top.u_core.u_pp.mem_a[pos][ch*8 +: 8];
        end
    endfunction
    ...
    function [7:0] read_mem_b;
        ...
        input integer pos;
        begin
            read_mem_b = u_top.u_core.u_pp.mem_b[pos][ch*8 +: 8];
        end
    endfunction
