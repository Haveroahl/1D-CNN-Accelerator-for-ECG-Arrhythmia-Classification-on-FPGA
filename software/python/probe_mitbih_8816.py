...
class ECG_1DCNN_5(nn.Module):
    def __init__(self, num_classes=5):
        super().__init__()
        self.conv1 = nn.Conv1d(1,  8, 5, padding=2, bias=True); self.pool1 = nn.MaxPool1d(4)
        self.conv2 = nn.Conv1d(8,  8, 5, padding=2, bias=True); self.pool2 = nn.MaxPool1d(4)
        self.conv3 = nn.Conv1d(8, 16, 5, padding=2, bias=True); self.pool3 = nn.MaxPool1d(2)
        self.conv4 = nn.Conv1d(16,16, 5, padding=2, bias=True); self.pool4 = nn.MaxPool1d(2)
...
def main():
    ...
    L = []
    L.append("# PROBE D — MIT-BIH 5-class, WIDER conv 8-8-16-16, FC 16->5, clean split\n")
    ...
    L.append(f"- Model 8-8-16-16 conv (2x wider), FC 16->5 single layer, float32, {epochs} ep\n")
    ...
    L.append(f"- 4-4-8-8  FC 8->5 :   0.9742 acc / 0.9354 macro-F1 / A-F1 0.802\n")
    L.append(f"- 4-4-8-8  FC 8->16->5: 0.9715 acc / 0.9263 macro-F1 / A-F1 0.752\n")
    L.append(f"- 8-8-16-16 FC 16->5 (this): {acc:.4f} acc / {f1m:.4f} macro-F1 / A-F1 {f1pc[CLS2IDX['A']]:.3f}\n")
    L.append(f"- params this model: {nparams}\n")
    report = "".join(L)
    with open(os.path.join(OUT_DIR, 'REPORT_8816.md'), 'w') as f:
        f.write(report)
    print("\n" + report)
    print(f"Saved -> {os.path.join(OUT_DIR, 'REPORT_8816.md')}")
