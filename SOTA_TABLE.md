...
>
> **⚠️ Phạm vi (cập nhật 2026-07-30)**: dòng B1 (ours) = **bản RTL ROM trên DE10-Standard** —
> bản duy nhất được báo cáo. KHÔNG đưa vào bảng/chart: biến thể SIMD-20, cơ chế weight-load qua
> Avalon, port DE0-Nano/Cyclone IV. Số B1 đã đồng bộ với compile 2026-07-07 (2,120 ALM /
> Fmax 108.46 MHz) — số cũ 2,201 ALM / 104.85 MHz đã lỗi thời.
...
|---|---|---|---|---|---:|---:|---:|---|---|
| A1 | **Ours (2026)** | 1D-CNN pruned, INT8 P2-QAT | **Rhythm** (10s record) | **1-lead (II) / 4 (AFIB,GSVT,SB,SR)** | **640** | **94.65** | **0.9396** | bit-exact RTL, wearable | ✅ PAPER_DATA.md |
| A2 | Le et al. (LightX3ECG) ✅ | 1D-SEResNet (DSConv) + lead-attn, pruned 80% | **Rhythm** (record) | 3-lead (I,II,V1) / **4-superclass** | **5.31M** (6.52 MB) | 98.73 | 0.9718 | **cùng 4-superclass task**; nhưng 3-lead, 8300× params | ✅ arXiv 2207.12381v2 full-text |
...
- **A2 LightX3ECG = 5.31M params** (6.52 MB), cùng 4-superclass, 98.73%/F1 0.9718, nhưng 3-lead.
  → **Ta 640 params vs 5.31M ≈ 8,300× nhỏ hơn**, accuracy thấp hơn 4pp nhưng **single-lead + INT8 +
...
|---|---|---|---|---|---|---:|---:|---|---:|---|---|---|---|
| B1 | **Ours (2026)** | Cyclone V 5CSXFC6D6F31C6 | ECG / 1D-CNN INT8 | Chapman (4) | **Rhythm** (10s) | **94.65** (board 94.27) | **100** (Fmax **108.46**) | **52.16 µs** | **~19,200** (≈20,800 @Fmax) | **536.08 mW** ⚠️ (dyn 110.89) | **2,120 ALM / 3,158 Reg / 28 DSP / 20 M10K** | **27.96 µJ** ⚠️ | ✅ PAPER_DATA.md |
...
> ⚠️ **Power/energy của ta (B1) có confidence `Low`**: PowerPlay lấy được toggle rate từ mô phỏng cho
> I/O 100% và register 70.4%, nhưng **combinational chỉ 2.5%** (5053/5182 node phải nội suy). Đây là
> giới hạn cấu trúc của VCD mức RTL — sau khi Fitter gộp LUT, tên node combinational không còn ánh xạ
> về tên RTL. Nâng lên Medium/High cần gate-level sim (`.vo` + SDF), ngoài phạm vi. Khi đưa vào bài
> ghi rõ là **ước lượng**, mạnh ở so sánh tương đối (tĩnh↔động, DSP↔phần còn lại), không so trực
> tiếp pp-với-pp với số power của competitor (họ dùng flow/công cụ khác).
>
> 📌 **Số power đã đổi 805.53 → 536.08 mW (re-run 2026-07-31)** — không phải thiết kế đổi, mà VCD cũ
> sai chế độ: nạp `tb_top.vcd` từ 0 ns gồm cả reset + nhiều test case nạp SRAM qua Avalon (83 pin
> toggle trong khi lõi đứng yên). VCD mới (`tb_power_vcd.v`) dump đúng 5219 chu kỳ cửa sổ suy luận.
> Static gần như không đổi (413.84 → 412.12 mW, hợp lý vì static không phụ thuộc activity); toàn bộ
> chênh lệch ở dynamic (377.72 → 110.89 mW). Số "623 mW / 10.3 µJ / 95.6% toggle" cũ hơn nữa là
> **sai** (ghi từ ký ức, không có trong `.pow.rpt`).
>
> 💡 **Phân rã dynamic** (bán được cho C1): DSP **59.72 mW = 54%** dynamic, combinational 15.40,
> register 14.49, M10K 12.00, clock-enable 9.18. → power-of-2 rescale (0 DSP thay vì 4 DSP18) cắt
> đúng vào thành phần tốn nhất, không phải tối ưu ngoài lề.
>
...
87.42 GOPS/W. → Accuracy ta **94.65 vs Liu 92.95 (+1.7pp)** ở cùng dataset/task/device-family,
throughput cao hơn (~19.2k vs ~15.1k) ở freq gấp đôi — **nhưng KHÔNG bán claim "thắng accuracy"**
(Δ1.7pp không kèm CI/std, hai bên khác split/preprocessing). Đánh đổi: Liu power vẫn thấp hơn
(66 vs 536 mW) vì 50 MHz + fully-mapped không truy cập memory runtime — đây là điểm Liu mạnh, phải
thừa nhận thẳng. **Nhưng phải so đúng thành phần**: 412 mW trong 536 mW của ta là **static của die
Cyclone V SoC** (có ARM cứng), không phải do thiết kế; phần động ta thực sự tốn chỉ **110.89 mW**.
Liu dùng 5CSEBA6 (die nhỏ hơn, không SoC) nên phần lớn khoảng cách đến từ **chọn device**, không
phải hiệu quả kiến trúc. Note thêm Liu 66 mW là **dual-function CNN+HR** đo gộp, còn số của ta có
confidence Low (xem ⚠️ dưới bảng). **Điểm bán chắc nhất là area/params**: ALM/Reg của ta ít hơn
nhiều (folded 2,120 ALM = 5% vs Liu 51%). B6-B10
...
- **Direct competitor = Liu 2023 (B2)**: cùng model class (1D-CNN 4-layer), cùng Chapman 4-class,
  cùng họ Cyclone V → so sánh fair nhất. Điểm bán, xếp theo độ chắc chắn:
  (i) **8-PE channel-parallel (folded)** vs Liu **fully-mapped** → ít ALM/Reg hơn nhiều (5% vs 51%
  ALM; ~23× Reg theo Resoure_complare.txt), DSP xấp xỉ (28 vs ~44) — **đây là luận điểm mạnh nhất,
  số từ report Quartus cả hai bên**; (ii) round-half-up vs floor (+0.38%, C1 — đo nội bộ cùng điều
  kiện nên chắc); (iii) accuracy 94.65 vs 92.95 — **nêu như quan sát, KHÔNG như claim thắng** (thiếu
  CI/std, khác split). Thừa nhận thẳng: Liu power thấp hơn (66 mW vs 536) vì 50 MHz + fully-mapped
  no-memory-traffic + die nhỏ không SoC, và số power của ta confidence Low. Khi bị hỏi, tách rõ
  static 412 mW (thuộc về device) khỏi dynamic 110.89 mW (thuộc về thiết kế).
- **Params (Bảng A) là đòn bẩy mạnh nhất**: cùng 4-superclass, ta 640 vs LightX3ECG 5.31M (~8,300×),
...
- **Energy/inference**: ít paper báo µJ/inf (đa số báo W + GOPS/W). Ta có 27.96 µJ/inf (động:
  5.78 µJ) nhưng **confidence Low** → không nên làm trục chính; B4 Zhang 2.05 µJ/inf là ngoại lệ
  đáng chú ý — cần xác nhận #class.

---

## 2. Pareto chart — kế hoạch

- **Trục khuyến nghị**: x = **params** (log) hoặc latency (log); y = accuracy (%). **Không dùng
  energy làm trục chính** vì số power của ta confidence Low và đa số competitor không báo µJ/inf.
- **Điểm**: mỗi paper 1 điểm; ours = 1 điểm (bản ROM 8-PE trên DE10 — bản duy nhất trong phạm vi).
