// tb_bitexact1.v — Bit-exact single-sample checkpoint test for ecg_accelerator_top
//
// Chạy 1 mẫu ECG qua lõi và so 7 checkpoint bit-exact với golden Python:
//   input_int8, after_pool1..4, after_gap, logits_fc  → 7/7.
// Rút gọn từ tb_top.v (bỏ TC handshake bus, coverage FSM, config/weight-RAM).
//
// Chọn mẫu qua plusarg (mặc định 0):
//   vsim ... +SAMPLE=1   → nạp ecg_sample1.hex + golden/sample1/*.mem
//
...
module tb_bitexact1;
    ...
    // trên Wave (add wave -analog sim:/tb_bitexact1/ecg_probe). Mảng SRAM không
    ...
    initial begin
        ...
        if (!$value$plusargs("SAMPLE=%d", sample_n)) sample_n = 9;
        case (sample_n)
        ...
        $display("=== tb_bitexact1: 7-checkpoint bit-exact, sample %0d ===", sample_n);
