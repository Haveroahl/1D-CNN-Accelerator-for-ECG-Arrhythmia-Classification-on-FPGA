...
class ECG_1DCNN_5(nn.Module):
    def __init__(self, num_classes=5):
        ...
        # FC with hidden layer: 8 -> 16 (ReLU) -> num_classes
        self.fc1 = nn.Linear(8, 16, bias=True)
        self.fc2 = nn.Linear(16, num_classes, bias=True)
...
def main():
    ...
    L = []
    L.append("# PROBE C — MIT-BIH 5-class, 4-4-8-8 + FC HIDDEN (8->16->5), clean split\n")
    ...
    L.append(f"- Model 4-4-8-8 conv + FC 8->16(ReLU)->5, float32, {epochs} ep\n")
    ...
    L.append(f"- Leaky FC 8->5 (oversample-before-split): 0.9770 acc / 0.9771 macro-F1\n")
    L.append(f"- Clean FC 8->5 (no hidden):              0.9742 acc / 0.9354 macro-F1\n")
    L.append(f"- Clean FC 8->16->5 (this):               {acc:.4f} acc / {f1m:.4f} macro-F1\n")
    report = "".join(L)
    with open(os.path.join(OUT_DIR, 'REPORT_fc16.md'), 'w') as f:
        f.write(report)
    print("\n" + report)
